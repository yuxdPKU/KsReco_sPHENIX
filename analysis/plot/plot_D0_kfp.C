...
void plot_D0_kfp()
{
  ...
  TCut sicut("track_1_MVTX_nHits>2 && track_1_INTT_nHits>1 && track_2_MVTX_nHits>2 && track_2_INTT_nHits>1");

  TCut ptcut("track_1_pT > 0.5 && track_2_pT > 0.5");
  
  TCut paircut("D0_DIRA>0.5 && pow(D0_decayLength,2)>1e-5 && track_1_track_2_DCA<0.5");
  ...
  ntp_likesign->Add(Form("/sphenix/u/xyu3/workarea/KsReco_sPHENIX/analysis/all%d_kfp_D0_likesign.root",run));
  ntp_unlikesign->Add(Form("/sphenix/u/xyu3/workarea/KsReco_sPHENIX/analysis/all%d_kfp_D0_unlikesign.root",run));
  ...
  ntp_unlikesign->Draw("track_1_track_2_DCA>>hpdca", sicut+ptcut+"D0_DIRA>0.0 && pow(D0_decayLength,2)>1e-6");
  hpdca->Draw();

  c1->cd(3);
  TH1D *hcosth = new TH1D("hcosth","cosThetaReco",100,0.5,1);
  ntp_unlikesign->Draw("D0_DIRA>>hcosth", sicut+ptcut+"pow(D0_decayLength,2)>1e-6");
  hcosth->Draw();

  c1->cd(4);
  TH1D *hpath = new TH1D("hpath","projected path length",100,0,10);
  ntp_unlikesign->Draw("D0_decayLength>>hpath", sicut+ptcut+"D0_DIRA>0.0");
  hpath->Draw();

  c1->Update();
  c1->SaveAs("figure/plot1_quality_kfp_D0.pdf");
  ...
  TH1D *hm = new TH1D("hm","invariant mass",60,1.6, 2.2);
  ntp_unlikesign->Draw("D0_mass>>hm",trcut);
 
  int binlo = hm->FindBin(1.75);
  int binhi = hm->FindBin(1.95);

  TH1D *hmbg = new TH1D("hmbg","mass background",60,1.6, 2.2);
  ntp_likesign->Draw("D0_mass>>hmbg", trcut);
  ...
  TF1 *f = new TF1("f","[0]+gaus(1)",1.6,2.2);
  f->SetParameters(0.0, 100.0, 1.85, 0.5);
  hm->Fit("f","R");
  f->Draw("same");

  TF1 * fg = new TF1("fg","gaus(0)", 1.6, 2.2);
  ...
  std::cout << " fit signal integral " << fg->Integral(1.75, 1.95) * (60 / 0.2) << std::endl;

  TF1 * fe = new TF1("fe","[0]", 1.6, 2.2);
  fe->SetParameter(0, f->GetParameter(0));
  std::cout << " fit background integral " << fe->Integral(1.75, 1.95) * (60/ 0.2) << std::endl;

  TLine* one = new TLine(1.6, 0.0, 2.2, 0.0);
  one->Draw();

  c2->Update();
  c2->SaveAs("figure/plot2_mass_KFP_D0.pdf");
  ...
  ntp_unlikesign->Draw("D0_rapidity>>hrap",trcut);
  hrap->Draw();  
  ...
  ntp_unlikesign->Draw("track_1_track_2_DCA:track_1_pseudorapidity>>hpcar",trcut && "track_1_bunch_crossing !=0 && D0_mass > 1.75 && D0_mass < 1.95");
  hpcar->Draw("colz");  

  c3->cd(5);
  TH2D *hpcac = new TH2D("hpcac","DCA vs crossing",50, -100, 100,30, -1.0, 1.0);
  ntp_unlikesign->Draw("track_1_track_2_DCA:track_1_bunch_crossing>>hpcac",trcut && "track_1_bunch_crossing != 0 && D0_mass > 1.75 && D0_mass < 1.95");
  hpcac->Draw("colz");  

  c3->cd(6);
  TH2D *hpcap = new TH2D("hpcap","DCA vs pathlength",50, 0, 4,30, -1.0, 1.0);
  ntp_unlikesign->Draw("track_1_track_2_DCA:D0_decayLength>>hpcap",trcut && "D0_mass > 1.75 && D0_mass < 1.95");
  hpcap->Draw("colz");  

  c3->Update();
  c3->SaveAs("figure/plot3_rapidity_kfp_D0.pdf");

  TCanvas *c4 = new TCanvas("c4","momentum",150,50,600,600);
  c4->Divide(2,2); 
  c4->cd(1);
  TH2D *hpt = new TH2D("hpt","pT1 vs pT2",100, 0, 2,100, 0, 2.0);
  ntp_unlikesign->Draw("track_1_pT:track_2_pT>>hpt",trcut && "D0_mass > 1.75 && D0_mass < 1.95");
  hpt->Draw("colz");  

  c4->cd(2);
  TH1D *hmom = new TH1D("hmom","Ks momentum",100, 0, 3);
  ntp_unlikesign->Draw("D0_p>>hmom",trcut && "D0_mass > 1.75 && D0_mass < 1.95");
  hmom->Draw("colz");  

  c4->cd(3);
  TH1D *hpks = new TH1D("hpks","Ks pT",100, 0, 3.0);
  ntp_unlikesign->Draw("D0_pT>>hpks",trcut && "D0_mass > 1.75 && D0_mass < 1.95");
  hpks->Draw();  

  c4->Update();
  c4->SaveAs("figure/plot4_momentum_kfp_D0.pdf");
