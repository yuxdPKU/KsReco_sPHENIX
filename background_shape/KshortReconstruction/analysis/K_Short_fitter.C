...
void K_Short_fitter( bool ppSig = false )
{
  ...
  //path = "~/workarea/PhotonConv/KsNew/background_shape/KshortReconstruction/unlikesign/";
  path = "~/workarea/PhotonConv/KsNew/background_shape/KshortReconstruction/likesign/";
  inputFile = path + "all52911_kfp.root";  
  dataType = "pp";
  ...
  xAxisTitle = "M(#pi^{#pm} #pi^{#pm}) [GeV/#it{c}^{2}]"; 
  ...
  //cutStream << minMass << " <= " << branch << " && " << branch << " <= " << maxMass << " && 0.5 <= K_S0_decayLength && K_S0_decayLength <= 10 && K_S0_DIRA <= 0.9 && K_S0_DIRA >= 0.7 && min(abs(track_1_IP_xy), abs(track_2_IP_xy)) >= 0.7 && track_1_pT>0.3 && track_2_pT>0.3";
  cutStream << minMass << " <= " << branch << " && " << branch << " <= " << maxMass << " && 0.5 <= K_S0_decayLength && K_S0_decayLength <= 10 && K_S0_DIRA <= 0.9 && K_S0_DIRA >= 0.7 && min(abs(track_1_IP_xy), abs(track_2_IP_xy)) >= 0.7 && K_S0_pT>0.2";
  //cutStream << minMass << " <= " << branch << " && " << branch << " <= " << maxMass << " && 0.5 <= K_S0_decayLength && K_S0_decayLength <= 10 && K_S0_DIRA <= 0.9 && K_S0_DIRA >= 0.7 && min(abs(track_1_IP_xy), abs(track_2_IP_xy)) >= 0.7 && !(track_1_pT>0.3 && track_2_pT>0.3)";
  //cutStream << minMass << " <= " << branch << " && " << branch << " <= " << maxMass << " && 0.5 <= K_S0_decayLength && K_S0_decayLength <= 10 && K_S0_DIRA <= 0.9 && K_S0_DIRA >= 0.7 && min(abs(track_1_IP_xy), abs(track_2_IP_xy)) >= 0.7 && track_1_MVTX_nHits==0 && track_1_INTT_nHits==0 && track_2_MVTX_nHits==0 && track_2_INTT_nHits==0";
  ...
  */
  ...
  TText *pt_LaTexDate = ptDate->AddText("Run 52911");
  ...
  legend->AddEntry(frame->findObject("model_Norm[K_S0_mass]_Comp[K_S0GaussOne,background]"),"K_{S}^{0}#rightarrow #pi^{#pm} #pi^{#pm}","f");
