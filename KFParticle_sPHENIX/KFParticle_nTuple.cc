...
void KFParticle_nTuple::initializeBranches()
{
  ...
  m_tree->Branch(TString(mother_name) + "_SV_x", &m_calculated_mother_SV_x, TString(mother_name) + "_SV_x/F");
  m_tree->Branch(TString(mother_name) + "_SV_y", &m_calculated_mother_SV_y, TString(mother_name) + "_SV_y/F");
  m_tree->Branch(TString(mother_name) + "_SV_z", &m_calculated_mother_SV_z, TString(mother_name) + "_SV_z/F");
  ...
  if (m_has_intermediates_nTuple)
  {
    for (int i = 0; i < m_num_intermediate_states_nTuple; ++i)
    {
      ...
      m_tree->Branch(TString(intermediate_name) + "_SV_x", &m_calculated_intermediate_SV_x[i], TString(intermediate_name) + "_SV_x/F");
      m_tree->Branch(TString(intermediate_name) + "_SV_y", &m_calculated_intermediate_SV_y[i], TString(intermediate_name) + "_SV_y/F");
      m_tree->Branch(TString(intermediate_name) + "_SV_z", &m_calculated_intermediate_SV_z[i], TString(intermediate_name) + "_SV_z/F");
...
//std::cout<<"m_calculated_mother_x = "<<m_calculated_mother_x<<" y = "<<m_calculated_mother_y<<" z = "<<m_calculated_mother_z<<" r = "<<sqrt(m_calculated_mother_x*m_calculated_mother_x+m_calculated_mother_y*m_calculated_mother_y)<<std::endl;
  ...
  for (int k = 0; k < m_num_intermediate_states_nTuple; ++k)
  {
    ...
    m_calculated_intermediate_SV_x[k] = intermediateDecayVertex.GetX();
    m_calculated_intermediate_SV_y[k] = intermediateDecayVertex.GetY();
    m_calculated_intermediate_SV_z[k] = intermediateDecayVertex.GetZ();
...
//std::cout<<"motherDecayVertex + intermediateArray "<<k<<" x = "<<intermediateArray[k].GetX()<<" y = "<<intermediateArray[k].GetY()<<" z = "<<intermediateArray[k].GetZ()<<" r = "<<sqrt(pow(intermediateArray[k].GetX(), 2) + pow(intermediateArray[k].GetY(), 2))<<std::endl;
  }
...
//std::cout<<"motherDecayVertex + daughterArray "<<k<<" x = "<<daughterArray[iter].GetX()<<" y = "<<daughterArray[iter].GetY()<<" z = "<<daughterArray[iter].GetZ()<<" r = "<<sqrt(pow(daughterArray[iter].GetX(), 2) + pow(daughterArray[iter].GetY(), 2))<<std::endl;
    ++iter;
  }

  m_calculated_mother_SV_x = motherDecayVertex.GetX();
  m_calculated_mother_SV_y = motherDecayVertex.GetY();
  m_calculated_mother_SV_z = motherDecayVertex.GetZ();
  m_calculated_mother_SV_chi2_per_ndof = motherDecayVertex.GetChi2() / motherDecayVertex.GetNDF();
//std::cout<<"sv_radial_position x = "<<motherDecayVertex.GetX()<<" y = "<<motherDecayVertex.GetY()<<" z = "<<motherDecayVertex.GetZ()<<" r = "<<sqrt(pow(motherDecayVertex.GetX(), 2) + pow(motherDecayVertex.GetY(), 2))<<std::endl;
