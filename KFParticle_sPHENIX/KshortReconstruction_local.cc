...
int KshortReconstruction_local::process_event(PHCompositeNode* topNode)
{
  ...
  if (Verbosity() > 2) std::cout<<"KsReco - m_runNumber = "<<m_runNumber<<" , m_evtNumber = "<<m_evtNumber<<std::endl;
  ...
  for (auto tr1_it = m_svtxTrackMap->begin(); tr1_it != m_svtxTrackMap->end(); ++tr1_it)
  {
    ...
    if (Verbosity() > 2) std::cout<<"KsReco - track1 id "<<id1<<std::endl;
    if (tr1->get_quality() > _qual_cut)
    {
      if (Verbosity() > 2) std::cout<<"KsReco - track1 does not pass quality cut "<<tr1->get_quality()<<std::endl;
      continue;
    }
    if (Verbosity() > 2) std::cout<<"KsReco - track1 pass quality cut"<<std::endl;
    if (tr1->get_pt() < track_pt_cut)
    {
      if (Verbosity() > 2) std::cout<<"KsReco - track1 does not pass pt cut "<<tr1->get_pt()<<std::endl;
      continue;
    }
    if (Verbosity() > 2) std::cout<<"KsReco - track1 pass pt cut"<<std::endl;

    short int crossing1 = tr1->get_crossing();
    if (Verbosity() > 2) std::cout<<"KsReco - track1 crossing "<<crossing1<<std::endl;
    ...
    if (Verbosity() > 2) std::cout<<"KsReco - track1 pass mvtx requirement"<<std::endl;
    ...
    {
      if (Verbosity() > 2) std::cout<<"KsReco - track1 does not pass dca cut, dcaxy "<<dcaVals1(0)<<" dcaz "<<fabs(dcaVals1(1))<<std::endl;
      continue;
    }
    if (Verbosity() > 2) std::cout<<"KsReco - track1 pass dca cut"<<std::endl;
    ...
    for (auto tr2_it = std::next(tr1_it); tr2_it != m_svtxTrackMap->end(); ++tr2_it)
    {
      ...
      if (Verbosity() > 2) std::cout<<"KsReco - track2 id "<<id2<<std::endl;
      if (tr2->get_quality() > _qual_cut)
      {
        if (Verbosity() > 2) std::cout<<"KsReco - track2 does not pass quality cut "<<tr2->get_quality()<<std::endl;
        continue;
      }
      if (Verbosity() > 2) std::cout<<"KsReco - track2 pass quality cut"<<std::endl;
      if (tr2->get_pt() < track_pt_cut)
      {
        if (Verbosity() > 2) std::cout<<"KsReco - track2 does not pass pt cut "<<tr2->get_pt()<<std::endl;
        continue;
      }
      if (Verbosity() > 2) std::cout<<"KsReco - track2 pass pt cut"<<std::endl;

      short int crossing2 = tr2->get_crossing();
      if (Verbosity() > 2) std::cout<<"KsReco - track2 crossing "<<crossing2<<std::endl;
      ...
      if (Verbosity() > 2) std::cout<<"KsReco - track2 pass mvtx requirement"<<std::endl;
      ...
      {
        if (Verbosity() > 2) std::cout<<"KsReco - track2 does not pass dca cut, dcaxy "<<dcaVals2(0)<<" dcaz "<<fabs(dcaVals2(1))<<std::endl;
        continue;
      }
      if (Verbosity() > 2) std::cout<<"KsReco - track2 pass dca cut"<<std::endl;
      ...
      if (Verbosity() > 2) std::cout<<"KsReco - charge track1 "<<tr1->get_charge()<<" , track2 "<<tr2->get_charge()<<std::endl;
      /*
      if (!_likesign && tr1->get_charge() == tr2->get_charge())
      {
        continue;
      }
      else if (_likesign && tr1->get_charge() != tr2->get_charge())
      {
        continue;
      }
      */
      if (Verbosity() > 2) std::cout<<"KsReco - pass charge requirement"<<std::endl;
      ...
      {
        if (Verbosity() > 2) std::cout<<"KsReco - pass pair dca cut "<<abs(pair_dca)<<std::endl;
        ...
        if (!ret1 or !ret2)
        {
          if (Verbosity() > 2) std::cout<<"KsReco - fail project track to point"<<std::endl;
          continue;
        }
        if (Verbosity() > 2) std::cout<<"KsReco - pass project track to point"<<std::endl;
        ...
        fillNtp(tr1, tr2, dcaVals1, dcaVals2, pca_rel1, pca_rel2, pair_dca, invariantMass, invariantPt, invariantPhi, rapidity, pseudorapidity, projected_pos1, projected_pos2, projected_mom1, projected_mom2, pca_rel1_proj, pca_rel2_proj, pair_dca_proj, track1_silicon_cluster_size, track2_silicon_cluster_size, track1_mvtx_cluster_size, track1_intt_cluster_size, track2_mvtx_cluster_size, track2_intt_cluster_size, m_runNumber, m_evtNumber);
...
bool KshortReconstruction_local::projectTrackToPoint(SvtxTrack* track, Eigen::Vector3d PCA, Eigen::Vector3d& pos, Eigen::Vector3d& mom)
{
  bool ret = true;
...
bool KshortReconstruction_local::projectTrackToCylinder(SvtxTrack* track, double Radius, Eigen::Vector3d& pos, Eigen::Vector3d& mom)
{
...
Acts::Vector3 KshortReconstruction_local::getVertex(SvtxTrack* track)
{
...
void KshortReconstruction_local::findPcaTwoTracks(const Acts::Vector3& pos1, const Acts::Vector3& pos2, Acts::Vector3 mom1, Acts::Vector3 mom2, Acts::Vector3& pca1, Acts::Vector3& pca2, double& dca)
{
  TLorentzVector v1;
  TLorentzVector v2;
...
KshortReconstruction_local::KshortReconstruction_local(const std::string& name)
  : SubsysReco(name)
{
}

Acts::Vector3 KshortReconstruction_local::calculateDca(SvtxTrack* track, const Acts::Vector3& momentum, Acts::Vector3 position)
{
...
int KshortReconstruction_local::End(PHCompositeNode* /**topNode*/)
{
  if (Verbosity() > 2) std::cout<<"my KshortReconstruction_local::End"<<std::endl;
  fout->cd();
  ntp_reco_info->Write();
  recomass->Write();
  fout->Close();
...
int KshortReconstruction_local::getNodes(PHCompositeNode* topNode)
{
  if (Verbosity() > 2) std::cout<<"my KshortReconstruction_local::getNodes"<<std::endl;
