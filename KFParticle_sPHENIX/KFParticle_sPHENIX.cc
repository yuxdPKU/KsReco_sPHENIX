...
{
std::cout<<"Hey! This is my local KFParticle_sPHENIX"<<std::endl;
...
int KFParticle_sPHENIX::process_event(PHCompositeNode *topNode)
{
  ...
  if (Verbosity() > 2) std::cout<<"KFP - begin"<<std::endl;
  if (Verbosity() > 2) std::cout<<"KFP - m_use_fake_pv "<<m_use_fake_pv<<std::endl;
  if (!m_use_fake_pv)
  {
    if (Verbosity() > 2) std::cout<<"KFP - m_use_mbd_vertex "<<m_use_mbd_vertex<<std::endl;
    if (m_use_mbd_vertex)
    {
  ...
  if (Verbosity() > 2) std::cout<<"KFP - pass vtx node"<<std::endl;
  ...
  if (Verbosity() > 2) std::cout<<"KFP - pass trk node"<<std::endl;

  multiplicity = check_trackmap->size();
  if (Verbosity() > 2) std::cout<<"KFP - trackmap size = "<<multiplicity<<std::endl;
