...
//Warning!!!!!!!!! here "false" is temporary for conversion analysis
//i use MDC2 dst truth-reco-matching-map
//but i make my own track fitting (TPC-only track), so track may not be truth-reco-matching-map
//therefore use method 2 to rebuild truth-reco-matching-map
//the result is the same but with higher memory and CPU usage
  //if (findNode && false)
  if (findNode)
  {
