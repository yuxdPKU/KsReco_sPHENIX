...
/*const*/ bool KFParticle_Tools::isGoodTrack(const KFParticle &particle, const std::vector<KFParticle> &primaryVertices)
{
  ...
  //std::cout<<"KFP - GoodTrackCheck track id "<<particle.Id()<<std::endl;
  if (pt >= m_track_pt && ptchi2 <= m_track_ptchi2 && min_ip >= m_track_ip && min_ipchi2 >= m_track_ipchi2 && trackchi2ndof <= m_track_chi2ndof)
  {
    //std::cout<<"KFP - pass GoodTrackCheck "<<std::endl;
    goodTrack = true;
  }
  else
  {
    //std::cout<<"KFP - fail GoodTrackCheck pt "<<pt<<" ptchi2 "<<ptchi2<<" min_ip "<<min_ip<<" min_ipchi2 "<<min_ipchi2<<" trackchi2ndof "<<trackchi2ndof<<std::endl;
  }
  return goodTrack;
}
...
std::vector<std::vector<int>> KFParticle_Tools::findTwoProngs(std::vector<KFParticle> daughterParticles, std::vector<int> goodTrackIndex, int nTracks)
{
  ...
  for (std::vector<int>::iterator i_it = goodTrackIndex.begin(); i_it != goodTrackIndex.end(); ++i_it)
  {
    for (std::vector<int>::iterator j_it = goodTrackIndex.begin(); j_it != goodTrackIndex.end(); ++j_it)
    {
      ...
      {
        float dca = 0;
	//std::cout<<"KFP - m_use_2D_matching_tools "<<m_use_2D_matching_tools<<std::endl;
	//std::cout<<"KFP - Id track1 "<<daughterParticles[*i_it].Id()<<" track2 "<<daughterParticles[*j_it].Id()<<std::endl;
        if (m_use_2D_matching_tools)
        {
	...
	//std::cout<<"KFP - dca "<<dca<<" DCAcut "<<m_comb_DCA<<std::endl;
        if (dca <= m_comb_DCA)
        {
	  //std::cout<<"KFP - pass dca cut"<<std::endl;
          ...
          {
            //std::cout<<"KFP - fail vertexchi2ndof = "<<vertexchi2ndof<<" cut "<<m_vertex_chi2ndof<<std::endl;
            continue;
          }
          else
          {
            //std::cout<<"KFP - satisfy vertexchi2ndof = "<<vertexchi2ndof<<" cut "<<m_vertex_chi2ndof<<std::endl;
            if (nTracks == 2 && sv_radial_position < m_min_radial_SV)
            {
              //std::cout<<"KFP - fail sv_radial_position = "<<sv_radial_position<<" cut "<<m_min_radial_SV<<std::endl;
              continue;
            }
            else
            {
              //std::cout<<"KFP - satisfy sv_radial_position = "<<sv_radial_position<<" cut "<<m_min_radial_SV<<std::endl;
        ...
        {
	  std::cout<<"fail dca cut "<<dca<<std::endl;
	}
      }
    }
  }
